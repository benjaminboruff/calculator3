...
pub fn Keypad<G: Html>(cx: Scope) -> View<G> {
    ...
    let row1KeySetView = View::new_fragment(
        ROW1.iter()
            .map(|&val| view! {cx, Key::<G>(key=val, value=val, attr:class="bg-blue-400 rounded-md min-w-full text-lg text-center text-white shadow-md"){} })
            .collect(),
    );
    ...
    let row4KeySetView = View::new_fragment(vec![
        view! {cx, Key::<G>(key="0", value="0", attr:class="bg-blue-400 rounded-md min-w-full text-lg text-center text-white shadow-md"){} },
        view! {cx, Key::<G>(key=".", value=".", attr:class="bg-blue-400 rounded-md min-w-full text-lg text-center text-white shadow-md"){} },
        view! {cx, Key::<G>(key="", value="", attr:class="bg-transparent rounded-md min-w-full"){} },
        view! {cx, Key::<G>(key="/", value="/", attr:class="bg-blue-400 rounded-md min-w-full text-lg text-center text-white shadow-md"){} },
    ]);

    let clearKey = View::new_fragment(vec![view! {cx,
        Key::<G>(key="clear", value="clear", attr:class="grow bg-red-400 rounded-md py-2 font-semibold text-center text-white shadow-md"){}
    }]);

    let undoKey = View::new_fragment(vec![view! {cx,
        Key::<G>(key="undo", value="undo", attr:class="grow bg-red-400 rounded-md py-2 font-semibold text-center text-white shadow-md"){}
    }]);

    let equalKey = View::new_fragment(vec![view! {cx,
        Key::<G>(key="=", value="=", attr:class="bg-green-400 rounded-md py-2 min-w-full text-3xl font-semibold text-center text-white shadow-md"){}
    }]);
    ...
    view! {cx,
        div(class="shadow-lg") {
            div(class="flex flex-col") {
                div(class="p-8 bg-blue-400 rounded-t-lg") {
